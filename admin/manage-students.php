    ...
    <div class="page-breadcrumb">
        <div class="row">
            <div class="col-12 align-self-center">
                <h2 class="mb-0 font-weight-bold mt-2">Manage Hostel Residents</h2>
            </div>
        </div>
    </div>
    ...
    <div class="container-fluid">
        <div class="card shadow-sm border-0">
            <div class="card-header">
                <h4 class="mb-0 font-weight-bold mt-2">Current Residents</h4>
            </div>
            <div class="card-body">

                <!-- Common Search Bar -->
                <input type="text" class="form-control search-box" id="searchAll" placeholder="Search">

                <!--------------------- BOYS TABLE ------------------------>
                <div class="card shadow-sm border-0 mt-4">
                    <div class="card-header">
                        <h5 class="mb-0 font-weight-bold mt-1">Boys Students</h5>
                    </div>
                    <div class="card-body">
                        ...
                        <table class="table table-hover table-bordered no-wrap" id="boysTable">
                            ...
                            <tbody>
                            <?php
                            $query = "SELECT * FROM registration ORDER BY roomno ASC";
                            $stmt = $mysqli->prepare($query);
                            $stmt->execute();
                            $res = $stmt->get_result();
                            $cnt = 1;
                            while($row = $res->fetch_object()) {
                                if(strtoupper(substr($row->roomno,0,1)) === 'B') {
                                    ?>
                                    <tr>
                                        <td><?php echo $cnt++; ?></td>
                                        ...
                                        <td><?php echo htmlentities($row->stayfrom); ?></td>
                                        <td><?php echo htmlentities($row->contactno); ?></td>
                                        <td class="actions-cell">
                                            <a href="students-profile.php?id=<?php echo htmlentities($row->id); ?>" class="btn btn-info btn-action"><i class="fas fa-user-circle"></i> View</a>
                                            <a href="manage-students.php?del=<?php echo htmlentities($row->id); ?>" class="btn btn-danger btn-action" onclick="return confirm('Are you sure you want to delete this record?');"><i class="fas fa-trash-alt"></i> Delete</a>
                                        </td>
                                    </tr>
                                    <?php
                                }
                ...
                <div class="card shadow-sm border-0 mt-5">
                    <div class="card-header">
                        <h5 class="mb-0 font-weight-bold mt-1">Girls Students</h5>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                        <table class="table table-hover table-bordered no-wrap" id="girlsTable">
                            ...
                            $cnt = 1;
                            while($row = $res->fetch_object()) {
                                if(strtoupper(substr($row->roomno,0,1)) === 'G') {
                                    ?>
                                    <tr>
                                        <td><?php echo $cnt++; ?></td>
                                        <td><?php echo htmlentities($row->regno); ?></td>
...
// Common search bar for both tables
